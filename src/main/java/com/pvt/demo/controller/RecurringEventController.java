...
public class RecurringEventController {
    ...
    public ResponseEntity<String> addRecurringEvent(@RequestBody RecurringEventDto dto) {
        ...
        if (organisation != null) {
            ...
            return ResponseEntity.ok("Recurring event '" + dto.name + "' added");
        } else {
            return ResponseEntity.badRequest().body("Organisation with ID " + dto.organisationId + " not found");
        }
    }

    @PutMapping("/updaterecurring/{id}")
    public ResponseEntity<String> updateRecurringEvent(@PathVariable Long id, @RequestBody RecurringEventDto dto) {
        ...
        if (event != null && organisation != null) {
            ...
            return ResponseEntity.ok("Recurring event with ID " + id + " updated successfully");
        } else if (event == null) {
            return ResponseEntity.badRequest().body("Recurring event with ID " + id + " not found");
        } else {
            return ResponseEntity.badRequest().body("Organisation with ID " + dto.organisationId + " not found");
        }
    }
    ...
    public ResponseEntity<String> deleteRecurringEvent(@PathVariable Long id) {
        RecurringEvent event = recurringEventRepository.findById(id).orElse(null);
        if (event == null) {
            return ResponseEntity.badRequest().body("Recurring event not found");
        }
        recurringEventRepository.delete(event);
        return ResponseEntity.ok("Recurring event deleted: " + event.getName());
    }
